...
def RMSD(native_structure, new_structure): 
    '''
    ...
    c = centroid(native_structure) 
    native_structure -= c
    c = centroid(new_structure)
    new_structure -= c
    ...
    for i in range(len(l_aligned)):
        square_sum += distance_squared(l_aligned[i],native_structure[i])
